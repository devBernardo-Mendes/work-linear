...
function maximizacao() {
  // Função para processo de maximização
  ...
  for (var i = 0; i < linhas; i++) {
    ...
    for (var j = 0; j < linhas + 3; j++) {
      // linhas + 3, pois é o valor de equações + x,y e RS
      ...
      if (j == 0)
        ...
        if (aux != i) {
          // Preenchendo os valores adicionais com 0 e 1 (diagonal principal)
          matriz[i][j] = 0;
        } else {
          matriz[i][j] = 1;
        }
  ...
  while (paraLoopMaximizacao(matriz[matriz.length - 1], vezesRodou)) {
    // Passando linha Z como parametro e vezes que rodou.

    //Escolhendo o termo de menor valor na linha z
    var valorminimo = Math.min(...matriz[linhas]);
    imprimirTexto(
      "O menor valor da coluna Z é " +
        valorminimo.toFixed(2) +
        ", sua coluna foi escolhida."
    );
  ...
  if (vezesRodou < 49) {
    ...
    textoFinal +=
      "  Z é: " + matriz[matriz.length - 1][linhas + 2].toFixed(2) + "\n";
...
function minimizacao() {
  // Função para processo de minimização
  ...
  while (paraLoopMinizacao(matriz, vezesRodou)) {
    // Passando linha Z como parametro e vezes que rodou.
    ...
    imprimirTexto(
      "O menor valor da coluna RS é " +
        valorminimo.toFixed(2) +
        ", sua linha foi escolhida."
    );
    ...
    for (var i = 0; i < matriz[linhaminimo].length; i++) {
      if (matriz[linhaminimo][i] == 0) vetAux.push(0);
      else {
...
function imprimir(matriz, linhas) {
  ...
  for (var i = 0; i < matriz.length; i++) {
    for (var j = 0; j < matriz[i].length; j++) {
      $("#resultado").html(
        $("#resultado").html() + "  " + matriz[i][j].toFixed(2) + "  "
      );
    }
...
function paraLoopMaximizacao(matriz, vezesRodou) {
  if (vezesRodou > 49) {
    imprimirTexto(
      "A função 50 vezes, por isso a execução será abortada, evitando loop infinito."
    );
    return false;
  }

  for (var i = 0; i < matriz.length; i++) {
    if (matriz[i] < 0) return true;
  }
...
function validaNumLinhas() {
  ...
  if (x1 != "" || y1 != "" || RS1 != "") numlinhas++;

  var x2 = $("#X2").val();
  var y2 = $("#Y2").val();
  var RS2 = $("#RS2").val();

  if (x2 != "" || y2 != "" || RS2 != "") numlinhas++;

  var x3 = $("#X3").val();
  var y3 = $("#Y3").val();
  var RS3 = $("#RS3").val();

  if (x3 != "" || y3 != "" || RS3 != "") numlinhas++;
